...
class down3:
    async def down(message : types.Message, state: FSMContext):
        '''нахождение всей литературы в разделе дневников и выдача пользователю'''
        async with aiohttp.ClientSession() as session:
            ...
            count = 0
            while j < 5:
                S = 'http://tolstoy.ru/creativity/diaries/?df=&dt=&q=&page='+str(j)
                async with session.get(S) as resp:
                    html1 = BS(await resp.text(), 'html.parser').find_all("a", text=re.compile("Дневник 1"))
                    html = BS(await resp.text(), 'html.parser').find_all("a", text=re.compile("Скачать "))
                    let = 0
                    for i in html1:
                        temp += repr(count + 1) + '.' + html1[let].text + '\n'
                        date += html[(let + 1) * 3 - 3].get("href") + ' '
                        date += html[(let + 1) * 3 - 2].get("href") + ' '
                        date += html[(let + 1) * 3 - 1].get("href") + ' '
                        count += 1
                        let +=1
                j = 1 + j
    ...
    def register_commands(dp: Dispatcher):
        '''обработчик команд'''
        dp.register_message_handler(down3.down, commands=['Diaries'])